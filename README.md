...
| 🔍 **インテリジェント検索** | ひらがな読みとレーベンシュタイン類似度で照合 | 「かとうなおこ」→「加藤直子」 |
...
### ✅ **認識成功例**
```
音声入力: "かとうなおこ" → ✅ 成功音 + 受付成功（ひらがな読み一致）
音声入力: "タナカタロウ" → ✅ 成功音 + 受付成功（カタカナ→ひらがな変換）
音声入力: "ささきまこと" → ✅ 成功音 + 受付成功（読みデータに一致）
```

### ❌ **認識失敗例**
```
音声入力: "かとう" → ❌ エラー音 + 読みの一部のみ
音声入力: "さとうはなこさん" → ❌ エラー音 + 余分な語尾で類似度低下
...
- **カスタムマッチングアルゴリズム** - ゲストのひらがな読み + レーベンシュタイン類似度
...
```typescript
{
  ...
  gender: "female",
  isPresent: false,
  reading: "かとうなおこ" // 音声認識結果と突き合わせる読み（ひらがな）
}
```

### **カスタマイズ方法**
1. `src/data/guests.ts` を編集
2. ゲスト名・性別に加えて `reading` をひらがなで設定（カタカナ入力も可）
3. 変更を保存すると即反映（ビルド不要、開発サーバーはホットリロード）
...
```typescript
// src/App.tsx
const MATCH_THRESHOLD = 0.9; // 0.0〜1.0で類似度のしきい値を設定
...
```
- `MATCH_THRESHOLD` を上げる → 認識に厳しくなる（誤認識を抑えたいとき）
- `MATCH_THRESHOLD` を下げる → 認識に寛容になる（多少の聞き間違いを許容したいとき）
